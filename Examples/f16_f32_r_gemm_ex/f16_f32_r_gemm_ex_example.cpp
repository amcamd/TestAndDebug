...
inline uint16_t float_to_half(float val) { return _cvtss_sh(val, 0); }

// Helper routine to convert halfs into their floats equivalent; uses F16C instructions
inline float half_to_float(uint16_t val) { return _cvtsh_ss(val); }
    ...
    const char* name, std::vector<T>& A, rocblas_int m, rocblas_int n, rocblas_int lda)
{
    ...
    for(int i = 0; i < m && i < max_size; i++)
    {
        for(int j = 0; j < n && j < max_size; j++)
        {
            ...
            { 
                std::cout << std::setw(4) << half_to_float(A[i + j * lda]) << " ";
            }
            else
            {
                  ...
                  int Ds2)
{
    for(int i1 = 0; i1 < M; i1++)
    {
        for(int i2 = 0; i2 < N; i2++)
        {
            ...
            {
                t += half_to_float(A[i1 * As1 + i3 * As2]) *
                     half_to_float(B[i3 * Bs1 + i2 * Bs2]);
            }
            D[i1 * Ds1 + i2 * Ds2] = float_to_half(beta * half_to_float(C[i1 * Cs1 + i2 * Cs2]) + alpha * t);
        }
    }
}
...
void initialize_a_b_c(std::vector<uint16_t>& ha,
                      rocblas_int size_a,
                      std::vector<uint16_t>& hb,
                      rocblas_int size_b,
                      std::vector<uint16_t>& hc,
                      rocblas_int size_c)
{
    for(int i = 0; i < size_a; ++i)
    {
        ha[i] = float_to_half(static_cast<float>(rand() % 2));
    }
    for(int i = 0; i < size_b; ++i)
    {
        hb[i] = float_to_half(static_cast<float>(rand() % 2));
    }
    for(int i = 0; i < size_c; ++i)
    {
        hc[i] = float_to_half(static_cast<float>(rand() % 2));
    }
}
...
int main(int argc, char* argv[])
{
    ...
    constexpr rocblas_datatype a_type       = rocblas_datatype_f16_r;
    constexpr rocblas_datatype b_type       = rocblas_datatype_f16_r;
    constexpr rocblas_datatype c_type       = rocblas_datatype_f16_r;
    constexpr rocblas_datatype d_type       = rocblas_datatype_f16_r;
    ...
    std::vector<uint16_t> ha(size_a), ha_packed(size_a);
    std::vector<uint16_t> hb(size_b), hb_packed(size_a);
    std::vector<uint16_t> hc(size_c);
    std::vector<uint16_t> hd(size_d);
    std::vector<uint16_t> hd_gold(size_d);
    ...
    if(verbose)
    {
        ...
        if(trans_a == rocblas_operation_none)
        {
            print_matrix<uint16_t>("ha initial", ha, m, k, lda);
        }
        else
        {
            print_matrix<uint16_t>("ha initial", ha, k, m, lda);
        }
        if(trans_b == rocblas_operation_none)
        {
            print_matrix<uint16_t>("hb initial", hb, k, n, ldb);
        }
        else
        {
            print_matrix<uint16_t>("hb initial", hb, n, k, ldb);
        }
        print_matrix<uint16_t>("hc initial", hc, m, n, ldc);
        print_matrix<uint16_t>("hd initial", hd, m, n, ldd);
    }
    ...
    if(verbose)
    {
        print_matrix<uint16_t>("hd_gold calculated", hd_gold, m, n, ldd);
        print_matrix<uint16_t>("hd calculated", hd, m, n, ldd);
    }
    ...
    for(int i_m = 0; i_m < m; i_m++)
    {
        ...
        {
            float error = half_to_float(hd_gold[i_m + i_n * ldd]) - half_to_float(hd[i_m + i_n * ldd]);

            error = error >= 0 ? error : -error;

            if(error != 0.0)
            {
                std::cout << i_m << ", " << i_n << ", " << half_to_float(hd_gold[i_m + i_n * ldd]) << ", " << half_to_float(hd[i_m + i_n * ldd]) << std::endl;
            }
