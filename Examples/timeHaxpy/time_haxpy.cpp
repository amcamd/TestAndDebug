...
int main(int argc, char *argv[]) 
{
    ...
    std::vector<__fp16> x(n*incx), y(n*incy);

    for(int i = 0; i < x.size(); i++) x[i] = __fp16((i+4)%4);
    for(int i = 0; i < y.size(); i++) y[i] = __fp16(i%2); 

    int size_x = n * incx * sizeof(__fp16);
    int size_y = n * incy * sizeof(__fp16);
    __fp16 *xd, *yd;
    ...
    for(int i = 0; i < number_outer_tests; i++)
    {
        ...
        {
            rocblas_haxpy(handle, n, &alpha, xd, incx, yd, incy);
        }
