...
//{
    ...
    template <typename T,
              ...
              typename TConstPtr,
              typename TPtr>
    __attribute__((amdgpu_flat_work_group_size(DIM_M * DIM_N, DIM_M* DIM_N))) __global__ void
        gemm_batched_general_kernel_new(rocblas_int    M,
                                    rocblas_int    N,
                                    rocblas_int    K,
                                    const T        alpha,
                                    TConstPtr*     dA_array,
                                    ...
                                    rocblas_int    batch_count)
    {
        ...
        auto* dA = dA_array + blz * stride_a;
        auto* dB = dB_array + blz * stride_b;
        auto* dC = dC_array + blz * stride_c;
                               ...
                               hipStream_t       stream)
    {
        ...
        else
        {
            ...
            if(beta == 0)
            {
                ...
                // clang-format off
                if(rocblas_operation_none == trans_a && rocblas_operation_none == trans_b)
                    hipLaunchKernelGGL((gemm_batched_general_kernel_new
                    <T, dim_m, dim_n, blk_m, blk_n, blk_k, blk_m, blk_k, blk_k, blk_n, true, 'N', 'N'>),
                    dimGrid, dimBlock, 0, stream, m, n, k, alpha, dA_array, lda, stride_a, dB_array, ldb, stride_b, beta, dC_array, ldc, stride_c, batch_count);
                if(rocblas_operation_transpose == trans_a && rocblas_operation_none == trans_b)
                    hipLaunchKernelGGL((gemm_batched_general_kernel_new
...
//  template
//  void syrkx_batched_solution<float>(rocblas_operation trans_a,
//                             rocblas_operation trans_b,
//                             rocblas_int       m,
//                             rocblas_int       n,
//                             rocblas_int       k,
...
//                             hipStream_t       stream);
    ...
    template 
    void gemm_batched_solution <float, float*, float*>(
