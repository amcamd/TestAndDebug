        ...
        T* c, rocblas_int ldc)
{
    ...
    return rocblas_status_success;
      ...
      template<typename T> rocblas_status trmm_reference (rocblas_side side, rocblas_fill uplo, rocblas_operation trans, rocblas_diag diag, rocblas_int m, rocblas_int n, T alpha, T *A, rocblas_int lda, T *B, rocblas_int ldb, T beta)
{
//
...
//
      if (alpha == 0)
      {
         gemm_reference(rocblas_operation_none, rocblas_operation_none, m, n, 0, 0.0, c, lda > ldc ? lda : ldc, c, lda > ldc ? lda : ldc, 0.0, c, ldc);
         return status;
      }
//
...
//
                        ...
                        else
                        {
                           ...
                           {
                              copy_reference(jsec, &c[i-1+(jj-1)*ldc], ldc, &t1[(i-ii)*ldt1__], 1);
                           }
                        }
//
...
//
                        for (int i = ii; i <= ii + isec -1; i++)
                        {
                           ...
                           {
                              delta = alpha * t2[i-ii +(i-ii)*ldt2__];
                           }
                           gamma = alpha;
                           tsec = ii+isec-1-i;
                           if (tsec == 0)
...
//                         gemv_reference(rocblas_operation_none, jsec, tsec, gamma, &t1[(i-ii+1)*ldt1__], rb, &t2[i - ii + 1 + (i - ii)*ldt2__], 1, delta, &t1[(i-ii)*ldt1__], 1);
                        }
//
...
//
                     if( ii+isec <= m)
                     {
                        gemm_reference(rocblas_operation_none, rocblas_operation_none, isec, n, m-ii-isec+1, alpha &a[ii-1+(ii+isec-1)*lda], lda, &c[ii+isec-1], ldc, 1.0, &c[ii-1], ldc);
                     }
                  }
      return rocblas_status_success;
}
...
///                           gamma = ZERO
///                        }
///                        CALL DGEMV ( 'N', JSEC, tsec, gamma, T1( 1, 1 ), RB, A( ii, i ), 1, delta, T1( 1, i-ii+1 ), 1 )
///80                   }
//
//                      C := T1', the transpose of T1 is copied back
//                      to C.
//
///                     for (int j = jj; j <= jj + jsec -1; jj++) {
///                        copy_reference(isec, &t1[j-jj], rb, &c[ii-1 + (j-1)*ldc], 1);
///90                   }
///00                }
//
...
//
///                  iF( ii.GT.1   {
///                     CALL DGEMM ( 'T', 'N', isec, N, ii-1, alpha, A( 1, ii ), lda, C( 1, 1 ), ldc, ONE, C( ii, 1 ), ldc )
///                  }
///10             }
///         }
///      } else {
