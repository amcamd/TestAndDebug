                   ...
                   bool verbose)
{
    ...
    if(verbose)
    {
        ...
        {
            print_matrix("a side_left, c <- alpha a b + beta c", ha, lda, m, lda);
        }
        else
        {
            print_matrix("a side_right, c <- alpha b a + beta c", ha, lda, n, lda);
        }
...
//      print_matrix("c_legacy", hc_legacy, ldc, n, ldc);
    ...
    status = symm_block_recursive( verbose, side, uplo, m, n, alpha,
        ha.data(), lda,
        hb.data(), ldb, beta,
