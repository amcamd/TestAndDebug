...
usage = \
''' A script to compare commits for ROCm libraries specified dkms-no-npi build.

Usage:
./compare-mainline.py -b build_baseline_num [-n build_new_num]
\t\t\t-h         \tdisplay Display help message
\t\t\t-b (int)   \tspecify Specify build baseline number
\t\t\t-n (int)   \tspecify Specify new build (default: latest)
'''
...
def gethashesfromurl(url):
    ...
    for child in root:
        ...
        if child.get('name') != None and child.get('revision') != None:
            dict[child.get('name')] = child.get('revision')
    return dict
...
def checkbuildhashes(baseline, newrev):
    #print("Checking build", str(baseline))

    #rev0 = getoldestsuccesflbuild()
    #print("First available build:  " + str(rev0))
    
    # if (rev0 > baseline):
    #     print("Build " + str(baseline) + " not available")
    #     sys.exit(0)
    ...
    url = "http://rocm-ci.amd.com/job/compute-rocm-dkms-no-npi-hipclang/" + str(baseline) + "/artifact/manifest.xml"
    basehashes = gethashesfromurl(url)
    #print( basehashes )

    url2 = "http://rocm-ci.amd.com/job/compute-rocm-dkms-no-npi-hipclang/" + str(newrev) + "/artifact/manifest.xml"
    newhashes = gethashesfromurl(url2)
    #print ( newhashes )

    for d in list(dictdiffer.diff(basehashes, newhashes)):         
        print(d)    
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv,"hb:n:")
    ...
    print("Comparing baseline (left) " + str(baseline) + " to build " + str(newrev) )

    checkbuildhashes(baseline, newrev)
